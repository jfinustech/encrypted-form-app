...
const SubmissionCard: React.FC<SubmissionCardProps> = ({ submission }) => {
    ...
    const handleDecrypt = async () => {
        ...
        try {
            ...
            const decrypted = await decryptData(submission.encryptedData, submission.iv, key);
    ...
    return (
        <div className="bg-card p-4 rounded-lg shadow-md space-y-3">
            <p className="text-sm text-text-secondary truncate">
                <span className="font-semibold text-text-primary">ID:</span> {submission.id}
            </p>
            <p className="text-sm text-text-secondary truncate">
                <span className="font-semibold text-text-primary">IV:</span> {submission.iv}
            </p>
            ...
            {decryptedContent && (
                 <div className="mt-4 p-4 bg-gray-50 rounded-md border border-gray-200 space-y-4">
                    ...
                    <div>
                        ...
                        <pre className="text-base text-text-primary whitespace-pre-wrap font-sans bg-white p-2 border rounded-md">{decryptedContent.message}</pre>
                    </div>
    ...
    <div className="space-y-4">
      {submissions.length === 0 ? (
        <div className="text-center py-10 px-4 bg-card rounded-lg shadow-md">
            <p className="text-text-secondary">No submissions yet. Fill out the form to create one.</p>
        </div>
      ) : (
